...
class gridsMAGEMin(object):
    """
    Class that helps with processing MAGEMin grid inputs when constructing a 
    phaseDiagram object. Designed to take the output constructed in the 
    notebook(s) packaged with pyMelt. The input table must conform to the
    style outlined in the import_pmelts notebook(s).

    Parameters
    ----------
    df : pandas.DataFrame
        The tabulated grid generated by MELTS
    phases : list of str, default: ['olv', 'cpx', 'opx', 'grt', 'spn', 'liq', 'plg']
        The phases that should be imported.
    oxides : dict
        The oxides to look for (as keys) with their molar masses.
    ...
    def __init__(self,
        ...
        # Prevent segmentation warning and improve efficiency:
    ...
    def calculate_phase_Mgn(self):
        ...
        for ph in self.phases:
            if ph + '_MgO_wtpt' in self.df.columns and ph + '_FeO_wtpt' in self.df.columns:
                mMgO = self.df[ph + '_MgO_wtpt'] / self.oxides['MgO']
                mFeO = self.df[ph + '_FeO_wtpt'] / self.oxides['FeO']
                self.df[ph + '_Mg#'] = mMgO / (mMgO + mFeO)
...
class gridsMelts(object):
    """
    Class that helps with processing MELTS grid inputs when constructing a 
    phaseDiagram object. Designed to take the output constructed in the 
    notebook(s) packaged with pyMelt. The input table must conform to the
    style outlined in the import_pmelts notebook(s).
    ...
    def __init__(self,
        ...
        # Relabel columns with oxides as wtpt:
        for col in df.columns:
            if col[:3] in self.phases and col[5:] in oxides.keys():
                df[col+'_wtpt'] = df[col]
...
class gridsThermocalc(object):
    """
    Class that helps with processing THERMOCALC grid inputs when constructing a 
    phaseDiagram object. Designed to take the input generated with:
    github.com/simonwmatthews/thermocalc_automation and processed using the
    notebooks packaged with pyMelt.
    ...
    def calculate_mass_fractions(self):
        """
        Converts the mole fractions provided as input into mass fractions of phases.
        """
