...
def matzen_D(state, mineral, element,
    ...
    if element == 'Mn':
        kd_olv = matzen_kd_olvliq_Mn(state, liq_MgO, **kwargs)
        if mineral == 'olv':
            kd_min = 1.0
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        elif mineral == 'opx':
            kd_min = matzen_kd_olvopx_Mn(state, **kwargs)
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        elif mineral == 'cpx':
            Dolv = matzen_D_from_kd(1.0, kd_olv, liq_MgO, olv_MgO, **kwargs)
            Dolvcpx = matzen_D_olvcpx_Mn(state, cpx_CaO)
            D = Dolv / Dolvcpx
        elif mineral == 'spn':
            kd_min = matzen_kd_olvspn_Mn(state, spn_Cr2O3, spn_Al2O3, spn_Fe2O3, **kwargs)
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        elif mineral == 'grt':
            kd_min = matzen_kd_olvgrt_Mn(state, **kwargs)
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        else:
            raise InputError("Mineral " + mineral + " not recognised.")
        
    elif element == 'Ni':
        kd_olv = matzen_kd_olvliq_Ni(state, liq_MgO, **kwargs)
        if mineral == 'olv':
            kd_min = 1.0
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        elif mineral == 'opx':
            kd_min = matzen_kd_olvopx_Ni(state, **kwargs)
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        elif mineral == 'cpx':
            kd_min = matzen_kd_olvcpx_Ni(state, **kwargs)
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        elif mineral == 'spn':
            kd_min = matzen_kd_olvspn_Ni(state, spn_Cr2O3, spn_Al2O3, spn_Fe2O3, **kwargs)
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        elif mineral == 'grt':
            kd_min = matzen_kd_olvgrt_Ni(state, **kwargs)
            D = matzen_D_from_kd(kd_min, kd_olv, liq_MgO, mineral_MgO, **kwargs)
        else:
            raise InputError("Mineral " + mineral + " not recognised.")
    else:
...
def matzen_D_from_kd(mineral_kd, olv_kd, liq_MgO=None, mineral_MgO=None, **kwargs):
    r"""
    ...
    return _np.exp(_np.log(DMg) + olv_kd - mineral_kd)


def matzen_kd_olvliq_Mn(state, liq_MgO=None, matzen_olvliq_Mn_A=0.0088, 
...
def matzen_kd_olvliq_Ni(state, matzen_olvliq_Ni_A=4505.0, 
...
def matzen_kd_olvopx_Mn(state, matzen_olvopx_Mn_A=-328.0, matzen_olvopx_Mn_B=-0.099,
                        **kwargs):
    r"""
...
def matzen_kd_olvopx_Ni(state, matzen_olvopx_Ni_A=-4171.0, matzen_olvopx_Ni_B=1.11,
                        **kwargs):
    r"""
...
def matzen_D_olvcpx_Mn(state, cpx_CaO=None,
...
def matzen_kd_olvcpx_Ni(state, matzen_olvcpx_Ni_A=-4171.0, matzen_olvcpx_Ni_B=1.11,
                        **kwargs):
    r"""
    ...
    state : pandas.Series
        The state of the system, e.g. temperature (T), pressure (P), melt fraction (F). This
        will most likely be generated automatically by the `MeltingColumn_1D` class.
    matzen_olvcpx_Ni_A : float, default: -4171.0
        Constant in the KD expression
    matzen_olvcpx_Ni_B : float, default: 1.11
        Constant in the KD expression
...
def matzen_kd_olvspn_Mn(state, spn_Cr2O3, spn_Al2O3, spn_Fe2O3, 
...
def matzen_kd_olvspn_Ni(state, spn_Cr2O3, spn_Al2O3, spn_Fe2O3, 
...
def matzen_kd_olvgrt_Mn(state, matzen_olvgrt_Mn_A=-4171.0, matzen_olvgrt_Mn_B=1.11,
                        **kwargs):
    r"""
...
def matzen_kd_olvgrt_Ni(state, matzen_olvgrt_Ni_A=5264.0, matzen_olvgrt_Ni_B=-2.065,
                        **kwargs):
    r"""
